...
if($_SESSION['securityLevel'] < 3){
    ...
    $responseArr = punchJobClock($employeeID, null, $punch, $userID, $departmentID);
    
    //Rebuild workorderpanel
    $htmlData = buildWorkOrderSection($originWONumber);
    $htmlData2 = buildUnassignedSection();
...
function buildWorkOrderView($buildOverlayResponseHTML = null){
    ...
    //Refresh Headers
    $productionQtyHeader = '<h2>Production Quantity  (' . $liveWorkOrderData[1] . ')</h2>';
...
function buildEmployeeScan($workOrderNumber){
    ...
    $departments = getDepartments();
     ...
     $html = '<ul>
                ...
                <li><label for="departmentID">Select Department:</label>
      ...
      $html .=  '</select></li>
                <li><label for="scanEmployee">Scan Employee #:</label>
...
function buildProductionSchedule($workOrderNumber){
    ...
    $productionSchedule = getProdSchedule('2017-7-11');
    ...
    $html = '<div class="grid">
                <div class="unit one-third">
                    <section>
                        ...
                        <h1>WorkOrder#: ' . $workOrderData[0][0] . '</h1>
                    </section>
...
function buildWorkOrderSummary(){
    ...
    $html .= '<div class="unit one-quarter">
      ...
      $html .= '</div>
                <div class="activeWO">
                    <h2 onclick="flip(\'productionSchedule\')">Unassigned</h2>';
    ...
    for($i = 0; $i < sizeof($unassignedEmployees); $i++){
        $html .= '<div class="woSummaryEmp" id="u' . $unassignedEmployees[$i]['employeeID'] . '" draggable="true" ondragstart="drag(event)" '
                . 'onclick="editEmployee(' . (isset($activeSummaryData[$i]['employeeID']) ? $activeSummaryData[$i]['employeeID'] : null) . ')">'
                        . $unassignedEmployees[$i]['name'] . ($unassignedEmployees[$i]['departmentName'] == null ? "" : ' - ' . $unassignedEmployees[$i]['departmentName'])
                        . '</div>';
    }
    
    $html .= '<h2 onclick="flip(\'productionSchedule\')">Clocked Out</h2>';
...
function buildWorkOrderSummarySubHeader($activeSummaryData, $j, $workOrderNumber = Null){
    ...
    $html = '<div class="woHeader" onclick="flip(\'w' . $activeSummaryData[$j]['workOrderNumber'] . '\')">
                                <div class="grid">
                                        <div class="unit one-third">
                                            <a href="/jobClock/index.php?action=workOrder&workOrderNumber=' 
                                        . $activeSummaryData[$j]['workOrderNumber']  . '">WO#: ' 
                                        . $activeSummaryData[$j]['workOrderNumber'] . '</a>
                                        </div>
...
// dragging unassigned employees to column for project
